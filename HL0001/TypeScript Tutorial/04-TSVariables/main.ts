...
let userName: string = 'Amit';  // userName is explicitly typed as string
const age: number = 25;         // age is explicitly typed as number and cannot be reassigned

// 2. Declare Type Without Value (Type Annotation Only)
// You can declare a variable with a type but without an initial value
let city: string;               // city is typed as string but has no initial value
...
let country = 'India';          // TypeScript infers country as type 'string'
...
let count = 5;
if (count > 0) {
  let message = "Count is positive";  // message is only accessible within this block
...
let userName1: string = "Arjun";   // Explicitly typed as string
let age1: number = 25;             // Explicitly typed as number
...
let greeting = greetUser(userName1, age1);
...
let globalVar = 10;  // globalVar is accessible throughout the entire program
